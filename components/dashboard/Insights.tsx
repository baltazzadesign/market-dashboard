...
function sessionLabel(session?: string) {
  if (session === "KRX_AFTER_MARKET") return "애프터마켓";
  if (session === "AFTER_HOURS_CLOSE") return "장후종가";
  if (session === "OLD_AFTER_HOURS_SINGLE_PRICE") return "시간외단일가";
...
export function Metrics({ rows, loading }: { rows: MarketRow[]; loading: boolean }) {
  ...
  const cards: { name: string; value: number | null | undefined; icon: IconName; digits?: number; unit?: string; signed?: boolean; foot: React.ReactNode; series: (number | null)[]; color?: string }[] = [
    { name: "KOSPI", value: last?.kospi, digits: 2, icon: "chart", foot: delta(last?.kospi, prev?.kospi), series: rows.map(r=>r.kospi), color: chartColors.green },
    { name: "KOSDAQ", value: last?.kosdaq, digits: 2, icon: "chart", foot: delta(last?.kosdaq, prev?.kosdaq), series: rows.map(r=>r.kosdaq), color: chartColors.violet },
    { name: "시장 폭", value: last?.diff, unit: "개", signed: true, icon: "activity", foot: <><span className="positive">상승 {fmt(last?.up)}</span><span>·</span><span className="negative">하락 {fmt(last?.down)}</span></>, series: rows.map(r=>r.diff), color: chartColors.yellow },
    { name: "외국인 + 기관", value: last?.flowPower, unit: "억", signed: true, icon: "layers", foot: <span>{last ? sourceLabel(last.flowSource) : "수급 기록 대기"}</span>, series: rows.map(r=>r.flowPower), color: "#9ca7b8" },
  ];
...
export function FlowPanel({ row }: { row?: MarketRow }) {
  ...
  return <section className="panel"><div className="panel-header"><h2 className="panel-title">투자자 수급</h2><span className="panel-subtitle">{row && sessionLabel(row.session) ? sessionLabel(row.session)+" · " : ""}누적 · 억원</span></div><div className="flow-list">{entries.map(e=><div className="flow-row" key={e.name}><div className="flow-row-head"><span style={{ display:"flex",gap:8,alignItems:"center" }}><span className="legend-line" style={{ color:e.color, width:8, height:8, borderRadius:2 }}/>{e.name}</span><strong className="num" style={{color:e.color}}>{fmt(e.value,0,true)}</strong></div><div className="flow-track" aria-hidden="true"><div className="flow-track-fill" style={{ width:Math.abs(e.value??0)/max*100+"%", background:e.color }}/></div></div>)}</div><div className="flow-total"><span className="muted">외국인 + 기관</span><strong className={"num " + valueClass(row?.flowPower)}>{fmt(row?.flowPower,0,true)}</strong></div>{row && row.flowSource !== "LIVE" && <div className="chart-footer"><Icon name="warning" size={14}/>{sourceLabel(row.flowSource)}</div>}</section>;
}

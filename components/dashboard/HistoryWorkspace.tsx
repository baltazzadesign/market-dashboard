...
function DayDetail({date,onClose}:{date:string;onClose:()=>void}){
  ...
  return <Modal open onClose={onClose} title={date+" · 장중 상세"} wide><div className="history-detail">
    <div className="toolbar"><div className="segmented">{([['flow','투자자 수급'],['kospi','KOSPI'],['kosdaq','KOSDAQ'],['breadth','시장폭'],['score','시장점수']] as [ChartKind,string][]).map(([key,label])=><button key={key} onClick={()=>setKind(key)} className={key===kind?"active":""} aria-pressed={key===kind}>{label}</button>)}</div><Link href={"/daily?date="+date} className="button small">일별 분석 열기</Link><Link href={"/daily?date="+date+"#closing-report"} className="button small">일일 리포트</Link></div>

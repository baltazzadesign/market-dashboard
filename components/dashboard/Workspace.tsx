...
export default function Workspace({ mode }: { mode:"overview" | "daily" }) {
  ...
  const [selectedMinute,setSelectedMinute] = useState<number | null>(null),[modal,setModal] = useState<"settings" | "guide" | "chart" | "board" | null>(null);
  ...
  function openChart(key:ChartKind){setReturnToBoard(modal==="board");setExpandedKind(key);setHoverMinute(null);setModal("chart");}
  ...
  return <div className="workspace">
    ...
    <Modal open={modal==="chart"} onClose={()=>{setModal(returnToBoard?"board":null);setReturnToBoard(false);}} title={chartLabels[expandedKind]+" · "+date} wide><div className="panel-header"><div className="chart-legend"><span className="panel-subtitle">연결된 시간 범위</span><strong className="num" style={{fontSize:14}}>{minuteLabel(domain[0])}–{minuteLabel(domain[1])}</strong></div>{ranges()}</div><MarketChart {...chartProps} kind={expandedKind} syncGroup="balta-fullscreen"/><div className="chart-footer"><span>Ctrl/⌘ + 휠 확대 · Esc로 닫기</span><button className="button small" onClick={()=>{setRange("all");setSelectedMinute(null);}}>전체 구간</button></div></Modal>

...
export default function SectorHeatmap(){
 ...
 return <section className="panel" style={{maxWidth:1400,margin:'18px auto 30px',padding:18}} aria-labelledby="sector-heatmap-title">
  <div className="panel-header" style={{display:'flex',gap:16,alignItems:'flex-start',justifyContent:'space-between',flexWrap:'wrap'}}>
   ...
   <div className="toolbar" style={{gap:8,flexWrap:'wrap'}}>
    <input aria-label="섹터 날짜" type="date" value={date} max={kstParts().date} onChange={e=>changeDate(e.target.value)}/>
  ...
  {error?<div role="alert" style={{padding:'20px 0'}}>{error}</div>:loading?<div style={{padding:'28px 0'}}>섹터 데이터 조회 중…</div>:!visible.length?<div style={{padding:'28px 0'}}>해당 날짜의 섹터 저장 데이터가 없습니다.</div>:<>
   ...
   <div style={{display:'grid',gridTemplateColumns:'minmax(0,2fr) minmax(260px,.8fr)',gap:14,alignItems:'start'}}>
    <div>
     <div style={{display:'grid',gridTemplateColumns:'repeat(6,minmax(0,1fr))',gridAutoFlow:'dense',gap:8}}>
      {visible.map(sector=>{
   ...
   <div style={{marginTop:12,fontSize:12,opacity:.65}}>저장 시각: {snapshotTime||'확인 불가'} · 섹터 {visible.length}개</div>
  </>}
 </section>;
}

...
export default function RecordsPanel({ rows, events, date, selectedMinute, onSelect, view, onViewChange }: {
  ...
  const [query, setQuery] = useState(""), [level, setLevel] = useState("전체"), [newest, setNewest] = useState(true), [page, setPage] = useState(0);
  useEffect(() => setPage(0), [query, level, date, view, newest]);
  ...
  const filteredEvents = useMemo(() => events.filter(event => (level === "전체" || event.level === level) && [event.time, event.label, event.message, event.source].join(" ").toLowerCase().includes(query.toLowerCase())), [events, query, level]);
  ...
  return <section id="records" className="panel" style={{ scrollMarginTop: 24 }} aria-labelledby="records-heading">
    ...
    <div className="table-tools"><label className="search-input"><Icon name="search" size={16}/><input value={query} onChange={event => setQuery(event.target.value)} placeholder={view === "records" ? "시간·수급 상태 검색" : "시간·신호·내용 검색"} aria-label="기록 검색"/></label>{view === "signals" && <select className="filter-select" aria-label="신호 강도" value={level} onChange={e => setLevel(e.target.value)}><option value="전체">모든 강도</option><option value="강">강한 신호</option><option value="중">중간 신호</option><option value="약">약한 신호</option></select>}<button className="button small" onClick={() => setNewest(v => !v)}><Icon name={newest ? "down" : "up"} size={14}/>{newest ? "최신순" : "시간순"}</button><button className="button small" disabled={!items.length} onClick={exportFiltered} title="현재 검색·필터에 해당하는 모든 기록 다운로드"><Icon name="download" size={15}/>CSV</button></div>

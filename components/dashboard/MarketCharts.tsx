    ...
    <div className={"chart-frame" + (compact ? " compact" : "")} role="img" aria-label={chartNames[kind] + " " + minuteLabel(domain[0]) + "부터 " + minuteLabel(domain[1]) + "까지의 추이. 상세 수치는 하단 기록 표에서 확인할 수 있습니다."}>
      <ResponsiveContainer width="100%" height="100%" minWidth={0}>
        <ComposedChart data={points} margin={{ top: 12, left: 3, right: 12, bottom: 4 }} syncId={syncGroup} syncMethod="value"
          ...
          <defs>{allSeries.map(s => <linearGradient key={s.key} id={id + s.key} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={s.color} stopOpacity={.17}/><stop offset="100%" stopColor={s.color} stopOpacity={.005}/></linearGradient>)}</defs>
          <CartesianGrid vertical={false} stroke="#263041" strokeDasharray="3 5"/>
          <XAxis dataKey="minute" type="number" domain={domain} ticks={ticks} tickFormatter={minuteLabel} tick={{ fill: "#8896ad", fontSize: 12 }} tickLine={false} axisLine={false} minTickGap={18} height={32} allowDataOverflow />
          <YAxis yAxisId="left" domain={fixedDomain} tick={{ fill: "#8896ad", fontSize: 12 }} tickFormatter={v => Math.abs(Number(v)) >= 10000 ? (Number(v)/1000).toFixed(0) + "k" : Number(v).toLocaleString("ko-KR", { maximumFractionDigits: 0 })} tickLine={false} axisLine={false} width={58} tickCount={5}/>
          ...
          {visible.map(s => single && kind !== "index" ? <Area key={s.key} yAxisId={s.axis ?? "left"} dataKey={s.key} name={s.name} type="linear" stroke={s.color} strokeWidth={2} fill={"url(#" + id + s.key + ")"} dot={false} activeDot={{ r: 4, stroke: "#11151e", strokeWidth: 2 }} connectNulls={false} isAnimationActive={false}/>
            : <Line key={s.key} yAxisId={s.axis ?? "left"} dataKey={s.key} name={s.name} type="linear" stroke={s.color} strokeWidth={2} dot={false} activeDot={{ r: 4, strokeWidth: 2, stroke: "#11151e" }} connectNulls={false} isAnimationActive={false}/>)}
